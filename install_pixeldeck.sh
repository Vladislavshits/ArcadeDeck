...
APP_PATH="$INSTALL_DIR/$APP_NAME.py"
GUIDES_JSON_PATH="$INSTALL_DIR/guides.json"
...
# Функция для создания файла приложения
create_app_file() {
    cat > "$APP_PATH" <<'PYTHON_CODE'
#!/usr/bin/env python3
import sys
import webbrowser
import os
...
# Функция для создания файла guides.json
create_guides_json() {
    cat > "$GUIDES_JSON_PATH" <<JSON
[
  {
    "title": "EmuDeck: Полное руководство по установке",
    "url": "https://example.com/emudeck"
  },
  {
    "title": "Настройка эмулятора Yuzu",
    "url": "https://example.com/yuzu"
  },
  {
    "title": "RPCS3 на Steam Deck",
    "url": "https://example.com/rpcs3"
  },
  {
    "title": "Оптимизация CEMU",
    "url": "https://example.com/cemu"
  },
  {
    "title": "Настройка эмулятора Dolphin",
    "url": "https://example.com/dolphin"
  },
  {
    "title": "Эмуляция PS2 с PCSX2",
    "url": "https://example.com/pcsx2"
  },
  {
    "title": "Vita3K для игр с PlayStation Vita",
    "url": "https://example.com/vita3k"
  },
  {
    "title": "Игры Switch на Steam Deck",
    "url": "https://example.com/switch"
  },
  {
    "title": "Эмуляция Xbox: руководство по Xemu",
    "url": "https://example.com/xemu"
  },
  {
    "title": "Настройка ядер RetroArch",
    "url": "https://example.com/retroarch"
  },
  {
    "title": "Настройка контроллера GameCube",
    "url": "https://example.com/gc_controller"
  },
  {
    "title": "Конфигурация Steam ROM Manager",
    "url": "https://example.com/srm"
  },
  {
  ...
  {
    "title": "Облачные сохранения для эмуляторов",
    "url": "https://example.com/cloud_saves"
  },
  {
    "title": "Сравнение эмуляторов Windows",
    "url": "https://example.com/windows_emu"
  }
]
JSON
}
...
# Запрос на установку
zenity --question --width=400 --title="Установка $APP_NAME" \
--text="Установить $APP_NAME? Программа будет установлена в:\n$INSTALL_DIR" \
...
(
    ...
    # Шаг 2/8: Создание файла приложения
    echo "20"
    echo "# Создание файла программы..."
    log "Создание файла $APP_PATH"
    create_app_file
    check_success "Создание файла программы"
    sleep 0.5

    # Шаг 3/8: Создание файла guides.json
    echo "30"
    echo "# Создание файла guides.json..."
    log "Создание файла $GUIDES_JSON_PATH"
    create_guides_json
    check_success "Создание файла guides.json"
    sleep 0.5

    # Шаг 4/8: Установка прав
    echo "40"
    echo "# Установка прав доступа..."
    log "Установка прав на исполнение для $APP_PATH"
    chmod +x "$APP_PATH"
    check_success "Установка прав на исполнение"
    sleep 0.5

    # Шаг 5/8: Проверка зависимостей
    echo "50"
    ...
    # Шаг 6/8: Создание ярлыка .desktop
    echo "80"
    echo "# Создание ярлыка приложения..."
    log "Создание .desktop файла: $DESKTOP_FILE"
...
EOF
    check_success "Создание .desktop файла"
    sleep 0.5

    # Шаг 7/8: Обновление меню
    echo "90"
...
# Перемещение и переименование установщика
INSTALLER_NEW_PATH="$INSTALL_DIR/Установщик PixelDeck.sh"
if [ -f "$INSTALLER_PATH" ]; then
    log "Перемещение установщика в $INSTALLER_NEW_PATH"
    mv "$INSTALLER_PATH" "$INSTALLER_NEW_PATH"
    chmod +x "$INSTALLER_NEW_PATH"
fi

# Финальное сообщение с кнопками запуска
response=$(zenity --question \
  --title="PixelDeck успешно установлен!" \
  --text="<b>PixelDeck успешно установлен!</b>\n\nПрограмму можно найти по пути:\n$INSTALL_DIR\n\nИли запустить её с панели приложений, введя \"PixelDeck\" в поиске.\n\nЗапустить программу сейчас?" \
