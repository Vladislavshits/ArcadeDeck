...
class Updater:
    ...
    def normalize_version(self, version_str):
        ...
        # Удаляем префикс 'v' и приводим к нижнему регистру
        clean_version = version_str.lstrip('v').lower()

        # Разделяем версию и метаданные (beta, rc и т.д.)
        version_parts = re.split(r'[\-+]', clean_version)
        main_version = version_parts[0]

        # Разбиваем основную версию на компоненты
        parts = []
        ...
        # Добавляем флаг бета (0 - стабильная, 1 - бета)
        is_beta = any('beta' in part for part in version_parts[1:])
        parts.append(0 if not is_beta else 1)
